...
contains
  ...
  subroutine rt_nlst_check(self)
   ...
   if( (self%water_temperature .lt. 0 ) .or. (self%water_temperature .gt. 1) ) then
      call hydro_stop('hydro.namelist ERROR: Invalid water_temperature specified')
   endif
  ...
  subroutine init_namelist_rt_field(did)
    ...
    integer:: RT_OPTION, CHANRTSWCRT, channel_option, &
         ...
         UDMP_OPT, io_form_outputs, bucket_loss, imperv_adj, water_temperature
...
!!! add the following two dummy variables
    ...
    namelist /HYDRO_nlist/ NSOIL, ZSOIL8,&
         ...
         io_config_outputs, io_form_outputs, hydrotbl_f, t0OutputFlag, output_channelBucket_influx, imperv_adj, water_temperature
...
! #endif
    ...
    nlst(did)%water_temperature = water_temperature
    ! files
